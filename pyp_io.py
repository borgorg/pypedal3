...
def a_inverse_to_file(pedobj, ainv=''):
    ...
    try:
        ...
        if not ainv:
            ainv = pyp_nrm.a_inverse_df(pedobj.pedigree)
        a_outputfile = '%s%s%s' % (pedobj.kw['filetag'], '_a_inverse_pickled_', '.pkl')
    ...
    try:
        ...
        for l in range(length):
            aout.write('%s,%s,%s,%s,%s,%s,%s\n' % (pedobj.pedigree[l].animalID, pedobj.pedigree[l].sireID,
                       pedobj.pedigree[l].damID, pedobj.pedigree[l].by, pedobj.pedigree[l].sex, pedobj.pedigree[l].fa,
                       pedobj.pedigree[l].gen))
        aout.close()
        _r = 1
    except:
        pass
    ...
    try:
        ...
        for l in range(length):
            if pedobj.pedigree[l].sex == 'm' or pedobj.pedigree[l].sex == 'M':
                ...
                sex = 2
            aout.write('%s %s %s %s %s %s %s\n' % (pedobj.pedigree[l].animalID, pedobj.pedigree[l].sireID,
                       pedobj.pedigree[l].damID, pedobj.pedigree[l].by, sex, '1', '1'))
        aout.close()
        _r = 1
    except:
        pass
    ...
    try:
        ...
        for l in range(length):
            ...
            if (mygen > 0) and (mygen <= 1.25):
                ...
                _gen = 0
            _maskgen = 1950 + _gen
            # Convert sexes from M/m (male) to 1 and F/f (female) to 2.
            if pedobj.pedigree[l].sex == 'm' or pedobj.pedigree[l].sex == 'M':
                sex = 1
            else:
                sex = 2
            aout.write('%s %s %s %s %s %s %s\n' % (pedobj.pedigree[l].animalID, pedobj.pedigree[l].sireID,
                                                   pedobj.pedigree[l].damID, _maskgen, sex, '1', '1'))
        aout.close()
        _r = 1
    except:
        pass
...
def summary_inbreeding(f_metadata):
    ...
    try:
        ...
        _summary = '\n%s\n%s' % (_summary, LINE2)
        for k,v in f_metadata['all'].items():
            _line = '\t%s\t%s' % (k,v)
            _summary = '%s\n%s' % (_summary, _line)
        _summary = '\n%s\n%s' % (_summary, LINE1)
        _summary = '%s\n%s' % (_summary, 'Animals with non-zero CoI:')
        _summary = '\n%s\n%s' % (_summary, LINE2)
        for k,v in f_metadata['nonzero'].items():
            _line = '\t%s\t%s' % (k,v)
            _summary = '%s\n%s' % (_summary, _line)
        _summary = '\n%s\n%s' % (_summary, LINE1)
        return _summary
...
##
# load_from_gedcom() reads and parses pedigree data that conform to a subset of the GEDCOM 5.5 specification. Not all
# valid GEDCOM are supported; unsupported tags are ignored.
...
def load_from_gedcom(infilename, messages='verbose', standalone=1, missing_sex='u',
    ...
    # Read the file
    if standalone == 0:
        logging.info('[load_from_gedcom]: Opening GEDCOM pedigree file %s.',infilename)
    ...
    try:
        while not all_done:
            next_zero = 0
            _curr_rec = []
            for l in range(zero_mark,len(inlines)+1):
            ...
            if firstlinelist[-1] == 'INDI':
                ...
                for c in range(1,len(_curr_rec)):
        ...
        if messages == 'verbose':
            print('[INFO]: Successfully imported pedigree from the GEDCOM file %s!' % infilename)
        logging.info('Successfully imported pedigree from the GEDCOM file %s!', infilename)
    except:
        if messages == 'verbose':
            print('[ERROR]: Unable to import pedigree from the GEDCOM file %s!' % infilename)
...
def save_from_gedcom(outfilename, assembled):
    """
    save_from_gedcom() takes pedigree data parsed by load_from_gedcom() and writes it to a text file in an ASD format
    that PyPedal can easily read.
    """
    pedformat = 'xxxx'
    try:
        ofh = open(outfilename,'w')
        for _i in list(assembled.keys()):
            pedformat = 'ASDxbu'
    ...
    try:
        ...
        for p in pedobj.pedigree:
            if p.sireID != pedobj.kw['missing_parent'] or p.damID != pedobj.kw['missing_parent']:
                if p.sireID not in par2spouses:
                    par2spouses[p.sireID] = []
                if p.damID not in par2spouses:
                    par2spouses[p.damID] = []
        ...
        for p in pedobj.pedigree:
            ...
            if p.animalID not in indi:
                ...
                if 'b' in pedobj.kw['pedformat'] and ( p.bd != pedobj.kw['missing_bdate'] and
                                                       p.bd != str(pedobj.kw['missing_byear'])):
                    indi[p.animalID] = '%s1 BIRT\n' % indi[p.animalID]
            ...
            if _fam not in fam and _fam[3:7] != 'F0_0':
                fam[_fam] = '0 @%s@ FAM\n' % _fam
            ...
            if 'WIFE' not in fam[_fam] and p.damName != pedobj.kw['missing_name']:
                fam[_fam] = '%s1 WIFE @%s@\n' % (fam[_fam], p.damName)
            fam[_fam] = '%s1 CHIL @%s@\n' % (fam[_fam], pedobj.namebackmap[pedobj.backmap[p.animalID]])
            # print fam[_fam]
        # Now loop and write the contents of indi and fam to the file.
        for i in list(indi.values()):
            ofh.write(i)
        for f in list(fam.values()):
            ofh.write(f)
        # Write footer and close file
        ofh.write('0 TRLR\n')
        ofh.close()
        if pedobj.kw['messages'] == 'verbose':
            print('[INFO]: Successfully exported pedigree to the GEDCOM file %s!' % outfilename)
        logging.info('Successfully exported pedigree to the GEDCOM file %s!', outfilename)
        return 1
    except:
        if pedobj.kw['messages'] == 'verbose':
            print('[ERROR]: Unable to save pedigree to the GEDCOM file %s!' % outfilename)
...
def load_from_genes(infilename, messages='verbose', standalone=1, missing_sex='u',
    ...
    # It looks like the GENES 1.20 site above has gone dormant. The material is preserved in the Internet
    # Archive: https://web.archive.org/web/20090505184718/http://www.vortex9.org/genes.html.
    ...
    for r in record_list:
        ...
        assembled[r[anidx]]['sire'] = r[field_list.index('SIRE_ID') - 1].strip()
        # "   UNK" = unknown ("  WILD" = founder)
        assembled[r[anidx]]['dam'] = r[field_list.index('DAM_ID') - 1].strip()
        # "   UNK" = unknown ("  WILD" = founder)
        ...
        # T/F indicating whether the animal is dead (T = dead)
        assembled[r[anidx]]['alive'] = r[field_list.index('DEAD') - 1]
    ...
    if messages == 'verbose':
        print('[INFO]: Successfully imported pedigree from the GENES 1.20 file %s!' % infilename)
...
def save_to_genes(pedobj, outfilename):
    ...
    # pyp2genes = { 'STUD_ID': 'originalID', 'SIRE_ID': 'sireName', 'DAM_ID': 'damName', 'STUD_ID': 'animalID',
    #               'BDATE': 'bd', 'SEX': 'sex', 'LOCATION': 'originalHerd', 'DEAD': 'alive', 'INBREED': 'fa',
    #               'AGE': 'age'
    # }
    pyp2genes = {'STUD_ID': 'animalID', 'SIRE_ID': 'sireName', 'DAM_ID': 'damName', 'BDATE': 'bd', 'SEX': 'sex',
                 'LOCATION': 'originalHerd', 'DEAD': 'alive', 'INBREED': 'fa', 'AGE': 'age'
                 }
    ...
    # If the pedigree could not be exported then tell the user that something went wrong.
        ...
        if pedobj.kw['messages'] == 'verbose':
            print('[ERROR]: Unable to export pedigree to the GENES file %s!' % outfilename)
...
def save_newanimals_to_file(animal_list, filename, pedigree, pedformat, kw):
    if len(animal_list) == 0:
        ...
        # First, save the unique animals from the union of pedigrees a and b based on the match rule. Note that the
        # pedformat from the first pedigree passed to __add__() will be used for both pedigrees. This makes sense
        # # because you cannot have two different pedformats in the same file.
        try:
            ofh = open(filename, 'w')
                ...
                for pf in pedformat:
                    ...
                    # This cascade may break if the pedigree is not renumbered...
                        ...
                        if _a.sireID != kw['missing_parent']:
                            value = pedigree[_a.sireID-1].originalID
                        else:
                            value = 0
                    elif pf in['d','D']:
                        if _a.damID != kw['missing_parent']:
                            value = pedigree[_a.damID-1].originalID
                        else:
                            value = 0
                    else:
                        value = getattr(_a, kw['new_animal_attr'][pf])
